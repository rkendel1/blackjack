...
class LatencyManager {
   ...
   */
  startMeasurement(intervalMs = 5e3) {
  ...
  getJitter() {
    ...
    const variance = this.latencyMeasurements.reduce((sum, val) => sum + Math.pow(val - avg, 2), 0) / this.latencyMeasurements.length;
...
class MatchmakingManager {
  ...
  matchFoundCallbacks = /* @__PURE__ */ new Map();
  /**
...
class AbusePreventionManager {
  sessionCreationLimits = /* @__PURE__ */ new Map();
  queueJoinLimits = /* @__PURE__ */ new Map();
  ...
  // 1 hour
  QUEUE_LIMIT_WINDOW_MS = 60 * 1e3;
  // 1 minute
  BLOCK_DURATION_MS = 15 * 60 * 1e3;
  // 15 minutes
  /**
  ...
  recordSessionCreation(userId, sessionId) {
    ...
    const sessions = this.activeSessions.get(userId) || /* @__PURE__ */ new Set();
...
class URLJoinManager {
  SECRET_KEY = "stacklive-multiplayer-secret";
...
class StackLiveMultiplayerRuntime {
  ...
  destroy() {
    ...
    this.stateSyncCallback = void 0;
  }
}
export {
  StackLiveMultiplayerRuntime as S
};
